...
abstract class TweetSet {
  ...
  def mostRetweetedHelper(biggest: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  override def filter(p: Tweet => Boolean) = this
  ...
  override def mostRetweetedHelper(biggest: Tweet) = biggest
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweetedHelper(biggest: Tweet) = {
    if (elem.retweets > biggest.retweets) right.mostRetweetedHelper(left.mostRetweetedHelper(elem))
    else right.mostRetweetedHelper(left.mostRetweetedHelper(biggest))
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(t.text.contains(_)))

  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(t.text.contains(_)))
